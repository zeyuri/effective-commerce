...
// biome-ignore lint/suspicious/noShadowRestrictedNames: effect has good reason to use this name
import { Effect, Function, flow, Layer, Schema } from 'effect'
import { DatabaseLive } from './database'

export const ProductGroupLive = HttpApiBuilder.group(ServerApi, 'product', (handlers) =>
  handlers
    .handle('createProduct', ({ payload }) =>
      Effect.gen(function* () {
        ...
        const InsertProduct = yield* SqlResolver.ordered('InsertProduct', {
          ...
          Result: Product,
          execute: (requests) => sql`INSERT INTO products ${sql.insert(requests)} RETURNING *`,
        })
    ...
    .handle('getProduct', ({ path }) =>
      Effect.gen(function* () {
        ...
        const GetById = yield* SqlResolver.findById('GetProductById', {
          ...
          Result: Product,
          ResultId: (_) => _.id,
          execute: (ids) => sql`SELECT * FROM products WHERE ${sql.in('id', ids)}`,
        })
